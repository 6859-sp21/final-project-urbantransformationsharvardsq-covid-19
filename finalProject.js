  ...
  if( tracks > 0){
      ...
      for (let walkIdx in allPoints){
        ...
        for (let ptIdx in allPoints[walkIdx]){
            stroke(clsFaded[i]);
            line(all_ln_glob_x[i], i*height/tracks + 6, all_ln_glob_x[i], (i+1)*height/tracks) - 6 ;
            ...
            if (mapInteractionLn){
                ...
                if (boxesClickedIdx-1 == i){
                    line(mapInteractionLn, i*height/tracks + 6, mapInteractionLn, (i+1)*height/tracks) - 6 ;
                }
                pop();
            }
            ...
            //
            push();
            var change_tt = analytic['change_tt'];
            var change_t = analytic['change_t'];
            ...
            var phase = allPoints[walkIdx][ptIdx]['geoPt']['properties']['phase'];
            ...
            if (phase['name']){
                ...
                poi[i][phase['name']] = all_ln_glob_x[i];
                stroke(clsFaded[i]);
                noFill();
